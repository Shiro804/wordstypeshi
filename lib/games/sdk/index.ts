...
/**
 * Utility: Generate a deterministic seed from inputs.
 * Used for daily challenges and reproducible games.
 */
export function generateDailySeed(gameId: string, mode: string, date: Date): string {
  const dateStr = date.toISOString().split('T')[0]; // YYYY-MM-DD
  return `${gameId}-${mode}-${dateStr}`;
}

/**
 * Utility: Simple seeded random number generator (Mulberry32).
...
/**
 * Utility: Pick N random items from an array using seeded random.
 */
export function pickRandomSeeded<T>(arr: T[], n: number, random: () => number): T[] {
  const result: T[] = [];
  const copy = [...arr];
